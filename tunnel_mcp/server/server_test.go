...
func TestServerAPI(t *testing.T) {
	...
	t.Run("REST Health Is Public On Routes And Daemon", func(t *testing.T) {
		...
		assert.Equal(t, "tunnel-mcp", health.Service, "Daemon health service should identify axolink")
	...
	t.Run("MCP Health Tool Returns Status", func(t *testing.T) {
		...
		assert.Equal(t, "tunnel-mcp", health.Service, "MCP health service should identify axolink")
...
func (s *fakeAgentStore) ListAgents(params agents.ListAgentParams) agents.ListAgentResponse {
	...
	for _, agent := range s.agents {
		...
		if params.Workspace != "" && agent.Info.WorkspaceDir != params.Workspace {
			continue
		}
